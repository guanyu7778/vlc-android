...
#endif
#if !defined(_WIN32)
    // Set recv buffer on non-Windows systems
    int rcvbuf_size = 10 * 1024 * 1024;
    if (setsockopt(fd, SOL_SOCKET, SO_RCVBUF, &rcvbuf_size, sizeof(rcvbuf_size)) == -1)
    {
        msg_Warn(p_obj, "Failed to set SO_RCVBUF: %s", vlc_strerror_c(errno));
    }
    else
    {
        int actual_size = 0;
        ...
        msg_Dbg(p_obj, "SO_RCVBUF set to %d bytes", actual_size);
    }
#endif
