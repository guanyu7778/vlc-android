...
static int SessionsSetup( demux_t *p_demux )
{
    ...
    while( ( sub = iter->next() ) != NULL )
    {
        ...
        else if( !strcmp( sub->mediumName(), "video" ) )
        {
            ...
            int i_recv_buf_size = var_InheritInteger( p_demux, "rtsp-receive-buffer-size" );
			if (i_recv_buf_size > 0)
				i_receive_buffer = i_recv_buf_size;
			else
				i_receive_buffer = 2000000; // fallback 默认值
			msg_Info( p_demux, "Set i_receive_buffer = %u bytes", i_receive_buffer );
        }
        ...
        else
        {
            if( sub->rtpSource() != NULL )
            {
				...
				msg_Info(p_demux, "Requested SO_RCVBUF = %u, actual = %d", i_receive_buffer, actual);
